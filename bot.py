...
# - drop_pending_updates=True to avoid old callbacks after restart
# - Safe callback query answering (ignore "Query is too old")
...
def run_flask():
    # Render sets PORT env var for web services
    port = int(os.environ.get("PORT", "10000"))
    app.run(host="0.0.0.0", port=port)

# --- Bot Settings (ENV only for token) ---
...
def search_jobs_logic(search_term: str, country_code: str):
    all_jobs = []
    if country_code == "all":
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Very important: callback queries can expire quickly.
    # If the bot restarts and re-processes old updates, this may fail.
    try:
        await query.answer()
    except BadRequest:
        return
...
async def perform_search(update_or_query, context: ContextTypes.DEFAULT_TYPE, search_term: str, country_code: str, is_callback: bool = False):
    ...
    if is_callback:
        await update_or_query.edit_message_text(
            f"🔍 جاري البحث عن <b>{escape_html(search_term)}</b>...",
            parse_mode=ParseMode.HTML,
        )
    ...
    if not results:
        await context.bot.send_message(chat_id, f"😔 لم أجد وظائف حالياً لـ {escape_html(search_term)}. حاول مرة أخرى بمسمى مختلف.")
        return
...
async def error_handler(update: object, context: ContextTypes.DEFAULT_TYPE) -> None:
    # Logs any unhandled errors. Keeps the bot alive instead of crashing.
...
def main():
    ...
    # Critical: avoid re-processing old updates after restart (prevents callback query expiry errors)
